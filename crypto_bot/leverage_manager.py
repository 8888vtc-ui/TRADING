...
class LeverageManager:
    ...
    def get_optimal_leverage_and_direction(self):
        if self.panic_mode:
            for (min_s, max_s), lev in self.short_matrix.items():
                if min_s <= self.fear_greed < max_s: return lev, TradeDirection.SHORT
            return 0, TradeDirection.NONE
        for (min_s, max_s), lev in self.long_matrix.items():
            if min_s <= self.unified_score < max_s: return lev, TradeDirection.LONG
        return 0, TradeDirection.NONE
    def can_use_leverage(self, signal, market_conditions=None):
        ...
        return LeverageDecision(can_leverage, optimal_lev if can_leverage else 1.0, level, direction, [f"{direction.value} {optimal_lev}x"], stop_loss_pct * stop_mult, 1.0, self.unified_score)
